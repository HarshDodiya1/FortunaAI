...
def create_final_dictionary(condensed_division_tables, department, college="LDRP-ITR"):
    final_dictionary = {
        college: {
            department: {}
        }
    ...
    for div_key, df in condensed_division_tables.items():
        ...
        if semester not in final_dictionary[college][department]:
            final_dictionary[college][department][semester] = {}
            
        # Initialize division
        if division not in final_dictionary[college][department][semester]:
            final_dictionary[college][department][semester][division] = {}
        ...
        for subject in df['Subject'].unique():
            ...
            final_dictionary[college][department][semester][division][subject] = subject_dict
            
    return final_dictionary

def final_func(matrix_file, department, college="LDRP-ITR"):
...
#     deptAbbreviation = request.form['deptAbbreviation']
#     if file and allowed_file(file.filename):
#         filename = secure_filename(file.filename)
#         filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
#         file.save(filepath)
        
#         results = final_func(filepath, college="LDRP-ITR", department=deptAbbreviation)
...
def upload_faculty_matrix():
    ...
    deptAbbreviation = request.form['deptAbbreviation']
    if file and allowed_file(file.filename):
        with tempfile.NamedTemporaryFile(suffix='.xlsx', mode='wb', delete=False) as temp_file:
            file.save(temp_file.name)
            try:
                results = final_func(temp_file.name, college="LDRP-ITR", department=deptAbbreviation)
                # Make the Node.js API call
                nodejs_response = requests.post(
                    'https://backend.reflectify.live/api/upload/faculty-matrix',
                    json=results,
                    headers={'Content-Type': 'application/json'}
                )
                return jsonify(results), 200
